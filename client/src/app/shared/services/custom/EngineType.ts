...
import { Car, LoopBackFilter, EngineType } from '../../../lib/models';
...
/**
 * Api services for the `EngineType` model.
 */
@Injectable()
export class EngineTypeApi extends BaseLoopBackApi {
   ...
   * This usually means the response is a `EngineType` object.)
   * </em>
   */
  public findByIdCars(id: any, fk: any) {
    ...
    return result.map((instance: EngineType) => new EngineType(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<EngineType>) =>
      instances.map((instance: EngineType) => new EngineType(instance))
    );
  }
