...
import { User, LoopBackFilter } from '../../../lib/models';
...
export class UserApi extends BaseLoopBackApi {
  ...
  */
  ...
  public login(credentials: any, include: any = 'user') {
    ...
    result.subscribe(
      (response: { id: string, userId: string, user: any }) => {
        // this.auth.update(response.id, response.userId, response.user);
        this.auth.update({ accessToken: response.id, username: response.user.username });
  ...
  public logout() {
    ...
    //  let urlParams: any = {};
    //  urlParams.access_token = this.auth.user.accessToken;
